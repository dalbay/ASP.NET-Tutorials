...
<%# Bind("UnitPrice", "{0:C}") %>
```  
**Exercise - Product List application**  
...
![datasource img](Images/dataSource8.png)   
Create the drop-down list and its data source
1. Open the XEx13CustomerList application in your exercises_extra directory. This application contains the starting page and the database, image, and style sheet used by the page.
2. Add a label to the form that contains the text shown above, followed by a non-breaking space and a drop-down list.
...
8. Display the data list in template-editing mode, and modify the Item template so it consists of a table with one row and three columns and that has a width of 600 pixels. Each cell in the row should include one of the labels that’s generated by default.
9. Set the width of the first two columns to 175 pixels, and set the width of the third column to 250 pixels.
10. Display the header template for the control, and add a table like the one you used for the Item template, but enter the values shown in the heading above.
Format the data list
11.	Use the AutoFormat dialog box to apply the Brown Sugar scheme to the data list. Then, set the BorderStyle property of the data list to Solid.
12.	Run the application to make sure it’s formatted properly. Select a different state from the drop-down list to make sure this works.
